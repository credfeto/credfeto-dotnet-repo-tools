...
public sealed class ReleaseGeneration : IReleaseGeneration
{
    ...
    private bool ShouldNeverReleaseFuzzyRules(in RepoContext repoContext, in BuildSettings buildSettings, in ReleaseConfig releaseConfig)
    {
        ...
        {
            return this.ReleaseSkipped(repoContext: repoContext, skippingReason: ReleaseSkippingReason.FOUND_PENDING_UPDATE_BRANCHES);
        }
        ...
        if (releaseConfig.CheckRepoForAllowedAutoUpgrade(repoContext.ClonePath))
        {
            ...
            return this.ReleaseSkipped(repoContext: repoContext, skippingReason: ReleaseSkippingReason.CONTAINS_PUBLISHABLE_EXECUTABLES);
        }

        return this.ReleaseSkipped(repoContext: repoContext, skippingReason: ReleaseSkippingReason.EXPLICITLY_PROHIBITED);
    }
