...
internal static partial class FileUpdaterLoggingExtensions
{
    [LoggerMessage(
        ...
        Message = "Checking: {sourceFileName} <-> {targetFileName}"
    )]
    private static partial void LogCheckingFile(
    ...
    [LoggerMessage(
        EventId = 2,
        Level = LogLevel.Information,
        Message = "{targetFileName} is already up to date with {sourceFileName}"
    )]
    private static partial void LogAlreadyUpToDate(
        this ILogger<FileUpdater> logger,
        string sourceFileName,
        string targetFileName
    );
    ...
    [LoggerMessage(
        ...
        Message = "{targetFileName} is different to {sourceFileName}, updating"
    )]
    [Conditional("DEBUG")]
    private static partial void LogTargetDifferent(
        this ILogger<FileUpdater> logger,
        string sourceFileName,
        string targetFileName
    );
    ...
    [LoggerMessage(
        EventId = 6,
        Level = LogLevel.Debug,
        Message = "{targetFileName} missing, copying from {sourceFileName}"
    )]
    [Conditional("DEBUG")]
    private static partial void LogTargetMissing(
        this ILogger<FileUpdater> logger,
        string sourceFileName,
        string targetFileName
    );
