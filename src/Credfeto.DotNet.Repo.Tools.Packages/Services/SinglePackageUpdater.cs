...
public sealed class SinglePackageUpdater : ISinglePackageUpdater
{
    ...
    {
        await EnsureCleanWorkingTreeAsync(repoContext: repoContext, cancellationToken: cancellationToken);

        await this.RequireSolutionBuildsBeforeUpdateAsync(
        ...
        try
        {
            return await this.ApplyPackageUpdateAsync(
                updateContext: updateContext,
                repoContext: repoContext,
                solutions: solutions,
                sourceDirectory: sourceDirectory,
    ...
    private static ValueTask EnsureCleanWorkingTreeAsync(
    ...
    )
    {
        ...
        return repoContext.Repository.ResetToDefaultBranchAsync(
            upstream: GitConstants.Upstream,
            cancellationToken: cancellationToken
        );
    }

    private async ValueTask<bool> ApplyPackageUpdateAsync(
        PackageUpdateContext updateContext,
        RepoContext repoContext,
        IReadOnlyList<string> solutions,
        string sourceDirectory,
    ...
    )
    {
        ...
        await this.OnPackageUpdateAsync(
            updateContext: updateContext,
            repoContext: repoContext,
            solutions: solutions,
            sourceDirectory: sourceDirectory,
    ...
    private async ValueTask RequireSolutionBuildsBeforeUpdateAsync(
        PackageUpdateContext updateContext,
        RepoContext repoContext,
        IReadOnlyList<string> solutions,
        string sourceDirectory,
    ...
    {
        string hash = await this._trackingHashGenerator.GenerateTrackingHashAsync(
            repoContext: repoContext,
            cancellationToken: cancellationToken
        );
    ...
    private async ValueTask OnPackageUpdateAsync(
        PackageUpdateContext updateContext,
        RepoContext repoContext,
        IReadOnlyList<string> solutions,
        string sourceDirectory,
    ...
    )
    {
        try
        {
            ...
            {
                string branchForUpdate = BranchNaming.BuildBranchForVersion(
                    branchPrefix: branchPrefix,
                    version: version
                );
