...
public sealed class BulkDependencyReducer : IBulkDependencyReducer
{
    ...
    private static class StaticMethods
    {
        ...
        public static bool IsDoNotRemovePackage(string packageId, HashSet<string> allPackageIds)
        {
        ...
        public static bool IsIgnoreProject(string projectName)
        {
        ...
        public static List<PackageReference> GetPackageReferences(string fileName, bool includeReferences, bool includeChildReferences)
        {
            ...
            if (includeReferences)
            {
                ...
                while (packageReferenceNodes.MoveNext())
                {
                    ...
                    if (IsDoNotRemovePackage(packageId: includeAttr, allPackageIds: allPackageIds))
                    {
                        continue;
                    }
        ...
        public static List<FileInfo> GetProjects(string sourceDirectory)
        {
            ...
            {
                bool ignoreProject = IsIgnoreProject(file.Name);
        ...
        public static int CheckReferences(string sourceDirectory)
        {
            List<FileInfo> files = GetProjects(sourceDirectory);
            ...
            foreach (FileInfo file in files)
            {
                ...
                byte[] rawFileContent = File.ReadAllBytes(file.FullName);
                ...
                List<PackageReference> childPackageReferences = GetPackageReferences(fileName: file.FullName, includeReferences: false, includeChildReferences: true);
                ...
                foreach (XElement node in nodes)
                {
                    ...
                    if (IsDoNotRemovePackage(packageId: includeAttr.Value, allPackageIds: allPackageIds))
                    {
                    ...
                    if (versionElement is not null)
                    {
                        ...
                        {
                            WriteProgress($"= {file.Name} references package {includeAttr.Value} ({versionElement.Value}) that is also referenced in child project {existingChildInclude.File.Name}.");
                            needToBuild = false;
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        if (existingChildInclude != null)
                        {
                            WriteProgress($"= {file.Name} references project {includeAttr.Value} that is also referenced in child project {existingChildInclude.File.Name}.");
                            needToBuild = false;
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                bool narrower = ShouldHaveNarrowerPackageReference(projectFolder: file.DirectoryName, packageId: packageId);
            ...
            foreach (ReferenceCheckResult obsolete in obsoletes)
            {
                ...
                if (StringComparer.Ordinal.Equals(obsolete.Type, "Package"))
                {
        ...
        private static bool IsDoNotRemovePackage(string packageId, List<string> allPackageIds)
        {
            throw new NotImplementedException();
        }
